...
def save_contacts():
    ...
    print("Save contacts")
    option = mazi_text(
        "Press 'p' to save file as a pickle otherwise save as JSON file: "
    )
    if option != "p":
        with open(file_name_1, "w") as out_file:
            json.dump(contacts, out_file)
    else:
...
def load_contacts():
    ...
    print("Load contacts")
    option = mazi_text(
        "Press 'p' to load file as a pickle otherwise load as JSON file: "
    )
    if option != "p":
...
file_name_1 = "smallcontacts.json"

# load_contacts(file_name_1)
